/*
 * name: budiono;
 * date: aug-08, 16:35, fri-2025; #66; 
 */

'use strict';

var RptTicketsSalesInvoicing={}
  
RptTicketsSalesInvoicing.table_name='rpt_tickets_sales_invoicing';
RptTicketsSalesInvoicing.title='Tickets Used in Invoicing';
RptTicketsSalesInvoicing.period=new PeriodLook(RptTicketsSalesInvoicing);

RptTicketsSalesInvoicing.show=(tiket)=>{
  tiket.modul=RptTicketsSalesInvoicing.table_name;
  tiket.menu.name=RptTicketsSalesInvoicing.title;
  tiket.rpt={
    "filter":{
      "period_id": "",
      "from":"",
  ...
  if(baru==-1){
    ...
    RptTicketsSalesInvoicing.preview(indek);
  }else{
    show(baru);
  }
}
...
RptTicketsSalesInvoicing.preview=(indek)=>{
  ...
  if(bingkai[indek].rpt.refresh==false){
    RptTicketsSalesInvoicing.proses(indek);
  } else {  
    RptTicketsSalesInvoicing.display(indek);
  };
};
...
RptTicketsSalesInvoicing.proses=(indek)=>{
  ...
  function getA(callback){
    var sql="SELECT customer_id,invoice_no"
      +",ticket_no,ticket_date,item_id,description,amount"
      +" FROM ticket_invoice_sum"
      +" WHERE company_id='"+bingkai[indek].company.id+"'"
      +" AND invoice_no !='' "
      +" AND ticket_date between '"+from+"' AND '"+to+"'"
    DownloadEmpat.run(indek,sql,(h)=>{
...
RptTicketsSalesInvoicing.display=(indek)=>{
  ...
  toolbar.refresh(indek,()=>{ RptTicketsSalesInvoicing.proses(indek); });
  toolbar.filter(indek,()=>{ RptTicketsSalesInvoicing.filter(indek); });
  toolbar.print(indek,()=>{ RptTicketsSalesInvoicing.print(indek); });
    ...
    +'<div class="a42" id="cetak2" style="margin:0 auto;">'// a4;
      ...
      +'<div style="position:sticky;width:210mm;margin-top:0;padding:0;">'// header
        +'<div style="width:100%;background:white;display:block;">'
          ...
          +s.setTitle( RptTicketsSalesInvoicing.title )
          ...
          +'<canvas id="cv_'+indek+'" width=800 height=25 style="position:absolute;border:1px solid lightgrey;"></canvas>'
            ...
            +s.setHeader(L[2], W[2], "left", 'Ticket No.')
            +s.setHeader(L[3], W[3], "left", 'Ticket Date.')
...
//--detail
      ...
      for(i=0;i<h2.length;i++){
        ...
        html+=''
          ...
          +s.setLabel(L[2], W[2], "left", h2[i].ticket_no )
          +s.setLabel(L[3], W[3], "left", tglWest(h2[i].ticket_date) )
...
// end-detail
  ...
  function sortByID(a,b){ // sort multidimensi;
    if( String(a.customer_id).concat(a.ticket_no) === String(b.customer_id).concat(b.ticket_no) ){
      return 0;
    }
    else{
      if( String(a.customer_id).concat(a.ticket_no) < String(b.customer_id).concat(b.ticket_no) ) {
...
RptTicketsSalesInvoicing.filter=(indek)=>{
  toolbar.none(indek);
  toolbar.hide(indek);
  toolbar.back(indek,()=>{ RptTicketsSalesInvoicing.preview(indek); });
  toolbar.preview(indek,()=>{ RptTicketsSalesInvoicing.filterExecute(indek); });
  RptTicketsSalesInvoicing.formFilter(indek);
};
...
RptTicketsSalesInvoicing.setPeriod=(indek,d)=>{
  var id_kolom=bingkai[indek].id_kolom;
  setEV(id_kolom, d.period_id);
  RptTicketsSalesInvoicing.getPeriod(indek);
};

RptTicketsSalesInvoicing.getPeriod=(indek)=>{
  RptTicketsSalesInvoicing.period.getOne(indek,
...
RptTicketsSalesInvoicing.filterExecute=(indek)=>{
...
RptTicketsSalesInvoicing.print=(indek)=>{};
