...
RptReimbursable.table_name='rpt_reimbursable';
RptReimbursable.title='Reimbursable Employee Expense';
RptReimbursable.period=new PeriodLook(RptReimbursable);

RptReimbursable.show=(tiket)=>{
  tiket.modul=RptReimbursable.table_name;
  tiket.menu.name=RptReimbursable.title;
  tiket.rpt={
    "filter":{
      "period_id": "",
      "from":"",
...
RptReimbursable.preview=(indek)=>{
  ...
  if(bingkai[indek].rpt.refresh==false){
    RptReimbursable.proses(indek);
  } else {  
    RptReimbursable.display(indek);
  };
};
...
RptReimbursable.display=(indek)=>{
  ...
  toolbar.refresh(indek,()=>{ RptReimbursable.proses(indek); });
  toolbar.filter(indek,()=>{ RptReimbursable.filter(indek); });
  toolbar.print(indek,()=>{ RptReimbursable.print(indek); });
    ...
    +'<div class="a42" id="cetak2" style="margin:0 auto;">'// a4;
      ...
      +'<div style="position:sticky;width:210mm;margin-top:0;padding:0;">'// header
        +'<div style="width:100%;background:white;display:block;">'
          ...
          +s.setTitle( RptReimbursable.title )
          ...
          +'<canvas id="cv_'+indek+'" width=800 height=25 style="position:absolute;border:1px solid lightgrey;"></canvas>'
            ...
            +s.setHeader(L[1], W[1], "left", 'AR Used')
            +s.setHeader(L[2], W[2], "left", 'Customer ID')
...
RptReimbursable.filter=(indek)=>{
  toolbar.none(indek);
  toolbar.hide(indek);
  toolbar.back(indek,()=>{ RptReimbursable.preview(indek); });
  toolbar.preview(indek,()=>{ RptReimbursable.filterExecute(indek); });
  RptReimbursable.formFilter(indek);
};
...
RptReimbursable.setPeriod=(indek,d)=>{
  var id_kolom=bingkai[indek].id_kolom;
  setEV(id_kolom, d.period_id);
  RptReimbursable.getPeriod(indek);
};

RptReimbursable.getPeriod=(indek)=>{
  RptReimbursable.period.getOne(indek,
...
RptReimbursable.filterExecute=(indek)=>{
