 ...
 * edit: apr-02, 17:08, thu-2026; #91; preview; bali:sanur;kute;ubud,bedugul;
 * edit: apr-04, 15:45, sat-2026; #91; preview;
 */
 
Quotes.preview=(indek,kursor)=>{
  
  if(kursor){// dari-paging
    Quotes.readOffset(indek,0,(d)=>{
      Quotes.renderPreview(indek,d);
    });
    
  } else {// dari-update
    
    Quotes.readKeyset(indek,(d)=>{
      Quotes.renderPreview(indek,d);
    });
  }
}
...
Quotes.renderPreview=(indek,d)=>{
  CompanyProfile.readCursor(indek,(c)=>{
    ...
    Quotes.cacheItem(indek,d,(h)=>{
      ...
      Quotes.binding(indek,total_halaman,jumlah_baris,c,d,arr);
    });
  });
}

Quotes.nextPrevious=(indek,val)=>{
  Quotes.readOffset(indek,val,(d)=>{
    if(bingkai[indek].metode==MODE_VIEW){// mode-preview
      ...
      Quotes.formUpdate(indek,d.customer_id,d.quote_no);
    }
  });  
}

Quotes.readKeyset=(indek,callback)=>{
  db.execute(indek,{
    query:"SELECT * "
      +" FROM quotes"
    ...
    Quotes.setFields(p,(d)=>{
      return callback(d);
    });
  });  
}

Quotes.readOffset=(indek,val,callback)=>{
  ...
  db.run(indek,{
    ...
    Quotes.setFields(p,(d)=>{// 
      ...
      bingkai[indek].quote_no=d.quote_no;
      return callback(d);
    });
  });
}

Quotes.setFields=(p,callback)=>{
  var m={
    customer_id: "",
    customer_name:"",
    customer_address:{},
...
Quotes.template=(indek,kopi,max_item,logo)=>{
  var lebar=678;
  var tinggi=0;
  var tinggi_top=0;
  var tinggi_a4=1000;
...
Quotes.cacheItem=(indek,s,callback)=>{
  var halaman=0;
...
Quotes.binding=(indek,total_halaman,jumlah_baris,data_c,data_s,arr)=>{
  // company
  var dc=data_c;
  var ds=data_s;
  ...
  for(var k=0;k<total_halaman.length;k++){
    ...
    // so_no, date, ship
    setiH('txt_quote_no_'+hal+'_'+indek, ds.quote_no);
    ...
    setiH('txt_good_thru_'+hal+'_'+indek, tglWest(ds.good_thru));
    setiH('txt_payment_terms_'+hal+'_'+indek, ds.discount_terms.displayed);
