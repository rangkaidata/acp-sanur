 ...
 * edit: mar-19, 09:33, thu-2026; #91; preview;
 * edit: mar-20, 16:33, fri-2026; #91; lebaran 2026;
 */
 
Invoices.preview=(indek,kursor)=>{

  if(kursor){// dari-paging
    Invoices.readOffset(indek,0,(d)=>{
      Invoices.renderPreview(indek,d);
    });
    
  } else {// dari-update
    
    Invoices.readKeyset(indek,(d)=>{
      Invoices.renderPreview(indek,d);
    });
  }
}
...
Invoices.renderPreview=(indek,d)=>{
  ...
  CompanyProfile.readCursor(indek,(c)=>{
    ...
    Invoices.cacheItem(indek,d,(h)=>{
      ...
      Invoices.binding2(indek,total_halaman,jumlah_baris,c,d,arr);
    });
  });
}

Invoices.nextPrevious=(indek,val)=>{
  Invoices.readOffset(indek,val,(d)=>{
    if(bingkai[indek].metode==MODE_VIEW){// mode-preview
      ...
      Invoices.formUpdate(indek,d.customer_id,d.invoice_no);
    }
  });
}

Invoices.readKeyset=(indek,callback)=>{
  db.execute(indek,{
    query:"SELECT * "
      +" FROM invoices"
    ...
    Invoices.setFields(p,(d)=>{// 
      return callback(d);
    });
  });
}

Invoices.readOffset=(indek,val,callback)=>{
...
Invoices.setFields=(p,callback)=>{
  ...
  if(p.count>0){
    ...
    m.paid_at_sale=d.paid_at_sale;
    m.note=JSON.parse(d.note);
    m.total=d.total;
    m.date_due=d.date_due;
    m.file_id=d.file_id;    
  }
...
Invoices.template=(indek,kopi,max_item,logo)=>{
  var lebar=678;
  var tinggi=0;
  var tinggi_top=0;
  var tinggi_a4=1000;
...
//--------------------------------------------------------------------//    
    for(var zz=0;zz<kopi;zz++){
      ...
      // title;
      +abc.teks(450,50+tinggi_top,'txt_title',30,'INVOICE','#555','bold')
      
      // number, date, ship, page;
      +abc.teks(450,65+tinggi_top,'lbl_invoice_no_'+zz+'_'+indek,12,'Invoice #:','#555','Normal')
      +abc.teks(540,65+tinggi_top,'txt_invoice_no_'+zz+'_'+indek,12,'INV1234','#555','Normal')
      ...
      +abc.teks(450,110+tinggi_top,'lbl_date_due_'+zz+'_'+indek,12,'Date Due:','#555','Normal')
      +abc.teks(540,110+tinggi_top,'txt_date_due_'+zz+'_'+indek,12,'31/03/2026','#555','Normal')
      +abc.teks(450,125+tinggi_top,'lbl_page',12,'Page:','#555','Normal')
      +abc.teks(540,125+tinggi_top,'txt_page',12,(zz+1)+' of '+kopi,'#555','Normal')      
...
Invoices.cacheItem=(indek,s,callback)=>{
  var halaman=0;
...
//  var data_c=c;
  ...
  var a=[];
  var b=data_s.so_detail;
  var c=data_s.detail;
  
  for(i=0;i<b.length;i++){
    if(b[i].description){// not undefined
      a.push({
        quantity: b[i].shipped,
        item_id: b[i].item_id,
        description: b[i].description,
        unit_price: b[i].unit_price,
        amount: b[i].amount,
      });
    }
  }
...
Invoices.binding2=(indek,total_halaman,jumlah_baris,data_c,data_s,arr)=>{
  // company
  var dc=data_c;
  var ds=data_s;
  ...
  for(var k=0;k<total_halaman.length;k++){
    ...
    // so_no, date, ship
    setiH('txt_invoice_no_'+hal+'_'+indek, ds.invoice_no);
    setiH('txt_date_'+hal+'_'+indek, tglWest(ds.date));
    setiH('txt_ship_date_'+hal+'_'+indek, tglWest(ds.ship_date));
    setiH('txt_date_due_'+hal+'_'+indek, tglWest(ds.date_due));
