 ...
 * edit: feb-05, 11:24, mon-2024; copy dari activity_item_look.js
 */ 
 
'use strict';

class ChargeItemLook{
  
  constructor(mother){
    this.mother=mother;
    this.url='charge_items';
    this.title='Charge Items';
    this.hideNew=true;
  ...
  readShow(indek){
    ...
    if(p.err.id!=0) bingkai.infoPaket(indek,p);
  ...
  addSelectFn(indek){
    ...
    for(var i=0;i<n.length;i++){
      ...
      n[i].onclick=function(){
        m.setChargeItem(p,d[this.dataset.i]);
        ui.CLOSE_POP(indek);
      }
    }
  }
