 ...
 * -----------------------------; happy new year 2026;
 * edit: apr-23, 12:20, thu-2026; #92; add next and previous;
 */ 
...
AccountBegins.readShow=(indek)=>{
  ...
  if (p.err.id===0){
    ...
    for (x in d){
      ...
      +'<td align="center">'+ribuan(d[x].debit)+'</td>'
      +'<td align="center">'+ribuan(d[x].credit)+'</td>'
      +'<td align="center">'
        +'<button type="button" '
        +' id="btn_change" '
  ...
  html+='<tr>'
      ...
      +'<td align="center"><strong>'+ribuan(debit_amt)+'</strong></td>'
      +'<td align="center"><strong>'+ribuan(credit_amt)+'</strong></td>'
      +'<td colspan="2">&nbsp;</td>'
    +'</tr>';
  html+='</table></div>';
...
AccountBegins.nextPrevious=(indek,val)=>{
  AccountBegins.readOffset(indek,val,(d)=>{
    AccountBegins.formUpdate(indek,d.account_id);
  });  
}

AccountBegins.readOffset=(indek,val,callback)=>{
  setCursor(indek,val);
  db.run(indek,{
    query:"SELECT * "
      +" FROM account_begins"
...
AccountBegins.setFields=(p,callback)=>{
  var m={
...
// EOF: 394;330;305;417;422;418;433;447;483;481;491;546;
