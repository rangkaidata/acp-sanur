 ...
 * -----------------------------; happy new year 2025;
 * edit: apr-23, 12:33, thu-2026; #92; add next dan previous;
 */ 
...
JournalEntry.calculate=(indek)=>{
  setiH('sum_debit_'+indek, ribuan(bingkai[indek].sum_debit) );
  setiH('sum_credit_'+indek, ribuan(bingkai[indek].sum_credit) );
  setiH(
    'is_balance_'+indek, 
...
JournalEntry.nextPrevious=(indek,val)=>{
  JournalEntry.readOffset(indek,val,(d)=>{
    JournalEntry.formUpdate(indek,d.journal_no);
  });  
}

JournalEntry.readOffset=(indek,val,callback)=>{
  setCursor(indek,val);
  db.run(indek,{
    query:"SELECT * "
      +" FROM journal_entry"
    ...
    JournalEntry.setFields(p,(d)=>{// 
      bingkai[indek].journal_no=d.journal_no;
      return callback(d);
    });
  }); 
}

JournalEntry.setFields=(p,callback)=>{
  var m={
    date: "",
    journal_no: "",
    description: "",
...
// eof: 577;514;628;642;660;699;702;709;706;715;753;
